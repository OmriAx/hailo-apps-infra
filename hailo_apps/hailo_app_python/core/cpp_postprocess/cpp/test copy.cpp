...
void ocr_postprocess(HailoROIPtr roi, void *params) {  
    ...
    // First, check if the ROI itself is a detection object (cascaded pattern)  
    auto roi_as_detection = std::dynamic_pointer_cast<HailoDetection>(roi);  
    if (roi_as_detection) {  
        // Attach classification directly to the ROI detection  
        auto classification = std::make_shared<HailoClassification>("text", result.text, result.confidence);  
        roi_as_detection->add_object(classification);  
        std::cout << "Added classification '" << result.text << "' to ROI detection" << std::endl;  
        return;  
    }  
    ...
    if (!detections.empty()) {  
        ...
        if (detection) {  
            ...
            if ( !result.text.empty()){
                ...
                std::cout << "Added classification '" << result.text << "' to ROI with confidence " << result.confidence << std::endl;
            }
        }  
} 
...
{  
    ...
    for (HailoDetectionPtr &detection : detections_ptrs) {  
        if (std::string("text") == detection->get_label()) {  
            ...
            // Clone the detection object to preserve it in the cropped ROI  
            HailoDetectionPtr new_roi = clone_detection_for_ocr(detection);  
            new_roi->set_bbox(bbox);  
              
            crop_rois.emplace_back(new_roi);  
              
            std::cout << "Cropped text region with detection preserved" << std::endl;  
        }  
    }  
